  ...
  switch (part.type) {
    ...
    default:
      ...
      if (part.type.startsWith('tool-')) {
        ...
        // Fallback for tools persisted as a whole part in `data_content`
        // (e.g. generateImage, document): return the original UI part as-is so
        // the stored output (image URL, etc.) is restored correctly.
        if (part.data_content) {
          return part.data_content as UIMessagePart
        }
...
export function buildUIMessageFromDB(
  ...
  // Merge metadata from DB with createdAt, the server-assigned sequence, and the
  // owning conversation id so the client can hydrate deterministically and never
  ...
  const metadata: UIMessageMetadata = {
    ...
    ...(typeof dbMessage.sequence === 'number' && {
