...
export function ChatHistoryClient() {
  ...
  const fetchInitialChats = useCallback(async () => {
    ...
    try {
      ...
      // De-duplicate by id in case the backend returns overlapping pages
      // (Firestore queries without a stable cursor can repeat documents).
      setChats(prevChats => {
        const seen = new Set(prevChats.map(c => c.id))
        const merged = [...prevChats]
